...
public sealed class AuthEndpointsTests : IClassFixture<CubotApiFactory>
{
    private readonly CubotApiFactory _factory;

    public AuthEndpointsTests(CubotApiFactory factory) => _factory = factory;

    [Fact]
    public async Task Login_SingleTenantUser_ReturnsTokenWithTenant()
    {
        var client = _factory.CreateClient();
        var token = await LoginAsync(client, CubotApiFactory.SingleEmail);
    ...
    public async Task Login_WrongPassword_Returns401()
    {
        ...
        var response = await client.PostAsJsonAsync(
            "/connect/token",
            new LoginRequest(CubotApiFactory.SingleEmail, "clave-incorrecta"));
    ...
    {
        var client = _factory.CreateClient();
        var token = await LoginAsync(client, CubotApiFactory.MultiEmail);
    ...
    public async Task TenantConfigurations_AreIsolatedByTokenTenant()
    {
        ...
        var tokenA = await LoginAsync(client, CubotApiFactory.SingleEmail);
        ...
        var tokenMulti = await LoginAsync(multi, CubotApiFactory.MultiEmail);
    ...
    {
        var client = _factory.CreateClient();
        var token = await LoginAsync(client, CubotApiFactory.SuperEmail);
        client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token.AccessToken);
    ...
    {
        var response = await client.PostAsJsonAsync("/connect/token", new LoginRequest(email, CubotApiFactory.Password, tenantId));
        response.EnsureSuccessStatusCode();
